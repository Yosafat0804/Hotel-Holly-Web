    ...
    <div class="card-body" style="padding:2rem;">
        <form action="{{ route('hotelfacility.update', $data->id) }}" method="post">
            ...
            <div class="form-group">
                ...
                <input id="facility_name" value="{{ $data->facility_name }}" name="facility_name" type="text" class="form-control @error('facility_name') is-invalid @enderror" value="{{ old('facility_name') }}" required autocomplete="facility_name" autofocus style="border-radius:16px; border:1.5px solid #d4af37;">
                @error('facility_name')
            ...
            <div class="form-group">
                <label for="detail" style="color:#00008b; font-weight:600;">Description</label>
                <textarea name="detail" class="form-control @error('detail') is-invalid @enderror" id="detail" rows="3" required autocomplete="detail" autofocus style="border-radius:16px; border:1.5px solid #d4af37;">{{ old('detail') }}{{$data->detail}}</textarea>
                @error('detail')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
